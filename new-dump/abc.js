...
// ---- HEADER ALIASES (exactly as you gave, with your tweak) ----
const A = {
  ...
  // Iqama / Passport (only _x per your file)
  iqamaNumber: ["Iqama Number_x"],
...
function validateRow(rowObj) {
  const missing = [];
  if (!rowObj.employeeNumber && !rowObj.email)
    missing.push("employeeNumber|email (need one as identifier)");
...
async function run() {
  ...
  for (const raw of rows) {
    ...
    if (!doc.employeeNumber && !doc.email) {
      failed.push({ employeeNumber: doc.employeeNumber, email: doc.email, missing, raw });
      continue;
    }
    ...
    // Upsert key preference
    const filter = doc.employeeNumber ? { employeeNumber: doc.employeeNumber } : { email: doc.email };

    // Do not overwrite with blanks / nulls / empty arrays
    // IMPORTANT: exclude validationStatus to avoid $set vs $setOnInsert conflict
