...
redisClient.connect().then(() => {
  console.log("Cron-2: Redis client connected for test cron job");
  ...
  const job = new Cron('*/20 * * * * *', async () => {
    ...
    try {
      ...
      while (retries < maxRetries) {
        ...
        if (acquired) {
          console.log('Cron-2: Test cron job running on this server at:', new Date());
          await new Promise(resolve => setTimeout(resolve, 3000)); // Simulate work for 3 seconds
          break; // Job executed, exit retry loop
        } else {
          console.log('Cron-2: Another server is running the cron job, skipping retries.');
      ...
      if (retries >= maxRetries) {
        console.error('Cron-2: Max retries reached, job not executed.');
      }
